...
class Human {
  ...
  void set_pos(int y, int x = 50, int offset = 0)
  {
    ...
    if (FLOORS::FIRST == y || FLOORS::THIRD == y || FLOORS::FIFTH == y) {
      ...
      x == 50 ? m_x = SCREEN_WIDTH - x - m_sprite.getGlobalBounds().width * (1 + offset) : m_x = x;
    }
...
class Elevator {
  ...
  UniqueQueue<FLOORS> m_path;
   ...
   public:
  static constexpr int MAX_HUMANS_ELEV = 8;
  ...
  {
    if (m_humans.size() < MAX_HUMANS_ELEV) {
  ...
  void return_check(sf::Clock &clock, bool areAllFloorsEmpty)
  {
    if (get_y() != FLOORS::FIRST) {
      if (areAllFloorsEmpty != true || m_humans.size() != 0) { clock.restart(); }
      else {
  ...
  bool moving(int dt)
  {
    if (m_rectangle.getPosition().y > m_Goal) { m_rectangle.move(0, -1); }
    if (m_rectangle.getPosition().y < m_Goal) { m_rectangle.move(0, 1); }
    if (m_rectangle.getPosition().y == m_Goal) { return false; }
...
class Button {
  ...
  {
    if (mouse_pos.x > m_pos_x && mouse_pos.x < m_pos_x + m_width && mouse_pos.y > m_pos_y &&
        mouse_pos.y < m_pos_y + m_width) {
...
inline Buttongroup make_buttons(int pos_x, FLOORS floor, sf::Color color)
{
  ...
  return Buttongroup{ Button(pos_x, floor, FLOORS::FIRST, color, '1'),
...
struct Floor {
      ...
      is_left(is_left), color(color), m_border(_border)
  {
    ...
    if (is_left)
      ...
      rectangle.setPosition(SCREEN_WIDTH - pos_x - m_width, pos_y + ELEVATOR_HIGHT);
  }
...
class Counter {
  ...
  Counter(int pos_x, int pos_y, sf::Color color)
  {
    ...
    m_color = color;
    m_rectangle.setSize(sf::Vector2f(m_width, m_hight));
    if (! m_font.loadFromFile("digital-7.ttf")) { std::cout << "Error loading font" << std::endl; }
    m_text.setFont(m_font);
    m_text.setCharacterSize(50);
    m_text.setFillColor(sf::Color::White);
    m_rectangle.setFillColor(m_color);
...
class ObjectManager {
  ...
  {
    for (auto &b : bg) {
      if (b.is_pressed(sf::Mouse::getPosition(m_window))) { spawn_human((FLOORS)b.m_beg, (FLOORS)b.m_goal); }
    }
  }
  void move_leftovers(float dt)
  {
  ...
  bool drop_off(FLOORS Goal)
  {
    ...
    for (auto &i : humans) {
      ...
      i->set_pos(Goal,
                 find_drop_off_point(Goal, m_elevator.get_x(), offset) + (offset * (n++ % Elevator::MAX_HUMANS_ELEV)));
      move_to_leftovers(i);
